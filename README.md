...
The separate Creative Formula converts the final evidence into a two-way Home-versus-Away view: `Home* = Home / (Home + Away)`, `Draw* = 0`, and `Away* = Away / (Home + Away)`. The displayed shares always total 100%. They are conditional shares answering which side is stronger if the match does not draw; they are not true 1X2 probabilities and do not mean the draw is impossible. The panel still shows the original draw evidence, applies a minimum 12-point Home/Away separation, checks the active odds band and context risk, and respects its own validation safety lock. Its leave-one-out check selected 110 matches, with 57 correct (51.8%), 20 draws and a flat-stake result of -0.20 units. Because this is not a positive validated return, the Creative Formula remains a research signal rather than a wager recommendation.
